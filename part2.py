...
def ResNet50():
    ...
    # Modify the first conv layer for 32x32 images
    model.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False) 
    model.maxpool = nn.Identity()  # Remove maxpool as we have smaller images (Recommendation from AI)
    num_ftrs = model.fc.in_features # Make the output have 100 features
...
def main():
    ...
    CONFIG = {
        "model": "ResNet50",   # Updated model name
        "batch_size": 128,     # Smaller batch size due to larger model
        "learning_rate": 0.001,
    ...
    # Update transforms to match ResNet preprocessing
    transform_train = transforms.Compose([
        transforms.RandomCrop(32, padding=4), # Crop the images
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)) # Using actual mean and standard deviation values from dataset
    ])
    ...
    optimizer = torch.optim.AdamW(model.parameters(), # Change to adamW to handle the adaptive learning rate better
